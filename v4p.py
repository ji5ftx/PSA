...
# 配置
FASTCHAT_API_URL = "http://localhost:8000" # fastchat API地址
...
class PromptStealingAttack:
    ...
    def analyze_samples(self, hit_samples, miss_samples):
        ...
        print(f"缓存命中统计:")
        print(f"  平均TTFT: {hit_mean:.6f}秒")
        print(f"  中位数TTFT: {hit_median:.6f}秒")
        print(f"  标准差: {hit_std:.6f}秒")
        print(f"缓存未命中统计:")
        print(f"  平均TTFT: {miss_mean:.6f}秒")
        print(f"  中位数TTFT: {miss_median:.6f}秒")
        print(f"  标准差: {miss_std:.6f}秒")

        # 计算两组样本的分离程度
        separation = (miss_median - hit_median) / ((hit_std + miss_std) / 2)
        print(f"分离度: {separation:.2f} (大于2表示良好分离)")
        #0.3的参数考虑？？？
        recommended_threshold = hit_median + (miss_median - hit_median) * 0.4

        # 验证数据的有效性，0.01的参数考虑？
        if (miss_median - hit_median) > 0.002:  # 至少10毫秒差异
...
def main():
    # 初始化PSA攻击
    psa = PromptStealingAttack(FASTCHAT_API_URL, MODEL_PATH)
